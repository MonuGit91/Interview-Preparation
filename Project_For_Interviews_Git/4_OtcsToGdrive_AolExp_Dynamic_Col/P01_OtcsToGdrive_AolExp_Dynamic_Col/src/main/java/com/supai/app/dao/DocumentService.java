...
//import com.supai.app.dao.dto.DTreeView;
...
//	private static final Logger log = LoggerFactory.getLogger(DocumentService.class); // @Slf4j will take care this line
	...
	public JsonNode getParentId(String docId) {
		try {
			String parentId = dtreeRepository.getParentIdByDataId(docId);	
	...
	//This method will one by one add new record or update old record wiht new if find
		public List<JsonNode> saveAllJsonDocsOrUpdateRow(List<JsonNode> jsonDocList) {
		    List<JsonNode> savedDocs = new ArrayList<>();

		    for (JsonNode jsonMetadata : jsonDocList) {
		        try {
...
//		            log.error("Failed to save or update document with Doc_Id: {}. Error: {}", doc.getDocId(), e.getMessage(), e);
	...
	public List<Document> saveAllDocsBachwise(List<Document> docList) {
		...
		for (int i = 0; i < newDocs.size(); i += batchSize) {
			...
			try {
				...
				log.info("Error saving batch from {} to {}: {}", i, end, e.getMessage(), e);
			}
		}
	...
	public List<Document> getAllDocuments() {
		try {
			...
			log.info("Error fetching documents: {}", e.getMessage(), e);
