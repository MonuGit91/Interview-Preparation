...
public class CallGDriveApi {
	private static final Logger log = LoggerFactory.getLogger(CallGDriveApi.class);
...
//						return (T) new ResponseEntity<>(jsonBody, headers, HttpStatus.UNAUTHORIZED);
...
//						System.exit(0);
				...
				if (e.getMessage().equalsIgnoreCase("Address already in use: bind")) {
					...
					log.error("either change the gdrive app-port.");
					log.error(
							"or refer https://docs.google.com/document/d/1asjf7jrPuD-WS5u1RrbUUXUcjGPtfK2KixSXHAw6Muo/edit?usp=sharing");
				} else
					log.error("{}", e.getMessage());
				return null;
			}
